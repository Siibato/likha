...
start_stack() {
  ...
  if [ -f /opt/likha/images/likha-server-arm64.tar ]; then
    docker load -i /opt/likha/images/likha-server-arm64.tar || log "WARN: docker load failed"
  fi
...
main() {
  ...
  if [[ -z "$node_id" ]]; then
    if command -v uuidgen >/dev/null 2>&1; then
      ...
      node_id="$(cat /proc/sys/kernel/random/uuid 2>/dev/null || openssl rand -hex 16 | sed 's/\(........\)\(....\)\(....\)\(....\)\(............\)/\1-\2-\3-\4-\5/')"
    fi
  fi
