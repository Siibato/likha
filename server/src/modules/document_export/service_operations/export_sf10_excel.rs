...
use crate::modules::document_export::helpers::excel_engine::{
    data_fmt, header_fmt, subtitle_fmt, title_fmt, ExcelEngine,
};
...
pub async fn run(
    ...
    sheet
        .merge_range(
            ...
            1,
            7,
            &format!("School: {} — SY: {}", school_name, school_year),
            &subtitle_fmt(),
        )
        .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;

    sheet
        .write_with_format(3, 0, "Name", &header_fmt())
        .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
    sheet
        .merge_range(3, 1, 3, 7, &sf10.student_name, &data_fmt())
        .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;

    if let Some(lrn) = &sf10.lrn {
        sheet
            .write_with_format(4, 0, "LRN", &header_fmt())
            .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
        sheet
            .merge_range(4, 1, 4, 7, lrn, &data_fmt())
            .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
    }
    ...
    for record in &sf10.scholastic_records {
        ...
        sheet
            .merge_range(row, 0, row, 7, &section_label, &header_fmt())
            .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
        row += 1;
        ...
        for (col, h) in headers.iter().enumerate() {
            sheet
                .write_with_format(row, col as u16, *h, &header_fmt())
                .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
        }
        row += 1;
        ...
        for subject in &record.subjects {
            sheet
                .write_with_format(row, 0, &subject.class_title, &data_fmt())
                .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
            for (i, grade) in subject.term_grades.iter().enumerate() {
                if let Some(g) = grade {
                    sheet
                        .write_with_format(row, (i + 1) as u16, *g, &data_fmt())
                        .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
                } else {
                    sheet
                        .write_with_format(row, (i + 1) as u16, "-", &data_fmt())
                        .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
                }
            }
            if let Some(fg) = subject.final_grade {
                sheet
                    .write_with_format(row, 5, fg, &data_fmt())
                    .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
            } else {
                sheet
                    .write_with_format(row, 5, "-", &data_fmt())
                    .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
            }
            sheet
                .write_with_format(
                    row,
                    6,
                    subject.descriptor.as_deref().unwrap_or("-"),
                    &data_fmt(),
                )
                .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
            sheet
                .write_with_format(row, 7, &record.school_name, &data_fmt())
                .map_err(|e| AppError::InternalServerError(format!("Excel: {}", e)))?;
            row += 1;
        }
        row += 1;
