...
pub fn routes(material_service: Arc<LearningMaterialService>) -> Router {
    Router::new()
        ...
        .route("/classes/{class_id}/materials", get(handler::get_materials))
        .route("/materials/metadata", get(handler::get_materials_metadata))
        .route("/materials/{id}", get(handler::get_material_detail))
        .route("/materials/{id}", put(handler::update_material))
        .route("/materials/{id}", delete(handler::delete_material))
        .route("/materials/{id}/reorder", post(handler::reorder_material))
        .route(
        ...
        .route("/material-files/{file_id}", delete(handler::delete_file))
        .route(
