...
pub async fn upload_file(
        ...
        && submission.status != "submitted"
    {
        return Err(AppError::BadRequest(
            "Can only upload files to draft, returned, or submitted (before deadline) submissions"
                .to_string(),
        ));
    }
    ...
    if let Ok(Some(existing_path)) = assignment_repo
    ...
    if let Err(e) =
        file_service::write_file(&disk_path, &file_data, Some(file_encryption_key)).await
    {
