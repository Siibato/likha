...
pub async fn upload_file(
    ...
    if let Ok(Some(existing_path)) = assignment_repo.find_active_file_path_by_hash(&file_hash).await {
        let file = assignment_repo.save_file(
    ...
    let mut disk_path = PathBuf::from(file_storage_path);
    disk_path.push("submission_files");
    disk_path.push(&disk_filename);

    if let Err(e) = file_service::write_file(&disk_path, &file_data, Some(file_encryption_key)).await {
    ...
    match assignment_repo.save_file(
