...
pub async fn grade_submission(
    ...
    let grade_item_id = auto_populate::auto_populate_score(
        ...
        request.score as f64,
    )
    .await?;

    if let Some(inv) = invalidator {
        inv.invalidate_assignment_detail(submission.assignment_id)
            .await;
        if let Some(term) = assignment.term_number {
            inv.invalidate_class_grades(assignment.class_id, term).await;
            inv.invalidate_student_grades(assignment.class_id, submission.student_id, term)
                .await;
        }
    ...
    let (student_first_name, student_last_name) =
        assignment_repo.find_student_name(graded.student_id).await?;
