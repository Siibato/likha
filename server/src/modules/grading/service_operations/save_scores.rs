...
impl crate::modules::grading::service::GradeComputationService {
    pub async fn save_scores(
        ...
        tracing::info!(
            "save_scores: grade_item_id={} scores_count={}",
            grade_item_id,
            scores.len()
        );
        ...
        self.repo
            .bulk_upsert_scores(grade_item_id, scores.clone())
            .await?;
        if let Some(ref inv) = self.invalidator {
            if let Some(ref item) = item {
                ...
                tracing::debug!(
                    "save_scores: invalidating cache for class_id={} term={}",
                    class_id,
                    term
                );
                let inv = inv.clone();
                ...
                tokio::spawn(async move {
                    ...
                    for (student_id, _) in &scores {
                        inv.invalidate_student_grades(class_id, *student_id, term)
                            .await;
                    }
                });
            }
