...
impl crate::modules::grading::service::GradeComputationService {
    pub async fn compute_class_term(
        ...
        let futures = enrolled_students.iter().map(|(student_id, _, _)| {
            self.compute_student_term(class_id, *student_id, term_number)
        });
        let result = try_join_all(futures).await?;
        if let Some(ref inv) = self.invalidator {
            inv.invalidate_class_grades(class_id, term_number).await;
            for (student_id, _, _) in &enrolled_students {
                inv.invalidate_student_grades(class_id, *student_id, term_number)
                    .await;
            }
        }
        Ok(result)
