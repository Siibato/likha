        ...
        match op.operation.as_str() {
            "create" => {
                ...
                match self.admin_service.create_account(request, user_id, client_id).await {
                    Ok(u) => success_result(op, Some(u.id.to_string()), Some(Utc::now().to_rfc3339())),
            ...
            "update" => {
                ...
                match action {
                    ...
                    _ => error_result(op, &format!("Unknown action for admin_user update: {}", action)),
                }
            }
            "delete" => {
                let target_user_id = match parse_uuid_field(&op.payload, "id") {
            ...
            _ => error_result(op, &format!("Unsupported operation for admin_user: {}", op.operation)),
        }
    }
}
