...
use crate::modules::assessment::schema::{
    AddQuestionRequest, ChoiceInput, EnumerationItemInput, UpdateQuestionRequest,
};
...
pub struct QuestionPushDelegate {
    pub assessment_service: Arc<AssessmentService>,
}

impl QuestionPushDelegate {
            ...
            arr.iter()
                .filter_map(|c| {
                    ...
                    let is_correct = c
                        ...
                        .unwrap_or(false);
                    let order_index =
                        c.get("order_index").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
                    Some(ChoiceInput {
                        choice_text,
                        is_correct,
                        order_index,
    ...
    fn parse_enumeration_items(payload: &serde_json::Value) -> Option<Vec<EnumerationItemInput>> {
        payload.get("enumeration_items")?.as_array().map(|arr| {
            arr.iter()
                .filter_map(|item| {
                    let acceptable_answers = item
                        .get("acceptable_answers")
...
#[async_trait]
impl PushDelegate for QuestionPushDelegate {
        ...
        match op.operation.as_str() {
            "create" => {
                ...
                let questions_array = match op.payload.get("questions").and_then(|v| v.as_array()) {
                    Some(arr) => arr,
                    None => return error_result(op, "Missing or invalid questions array"),
                };
                ...
                for q in questions_array {
                    ...
                    let question_type = match parse_str_field(q, "question_type") {
                        Ok(v) => v,
                        Err(e) => return error_result(op, &e),
                    };
                    let question_text = match parse_str_field(q, "question_text") {
                        Ok(v) => v,
                        Err(e) => return error_result(op, &e),
                    };
                    let points = match parse_i32_field(q, "points") {
                        Ok(v) => v,
                        Err(e) => return error_result(op, &e),
                    };
                    let order_index =
                        q.get("order_index").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
                    ...
                    let tos_competency_id = q
                        .get("tos_competency_id")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                    let cognitive_level = q
                        .get("cognitive_level")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                ...
                match self
                    .assessment_service
                    .insert_questions_for_assessment(assessment_id, questions, user_id)
                    .await
                {
                    Ok(mut responses) => match responses.pop() {
                        Some(r) => success_result(
                            op,
                            Some(r.id.to_string()),
                            Some(Utc::now().to_rfc3339()),
                        ),
                        None => error_result(op, "No questions returned from server"),
                    },
                    Err(e) => error_result(op, &e.to_string()),
                }
            }
            "update" => {
                let question_id = match parse_uuid_field(&op.payload, "id") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                ...
                match self
                    .assessment_service
                    .update_question(question_id, request, user_id)
                    .await
                {
            ...
            _ => error_result(
                op,
                &format!("Unknown operation for question: {}", op.operation),
            ),
        }
    }
}
