...
use crate::modules::assessment::schema::{AddQuestionRequest, UpdateQuestionRequest, ChoiceInput, EnumerationItemInput};
...
impl PushDelegate for QuestionPushDelegate {
    ...
    async fn process(
        ...
        match op.operation.as_str() {
            "create" => {
                ...
                match self.assessment_service.insert_questions_for_assessment(assessment_id, questions, user_id).await {
                    Ok(mut responses) => match responses.pop() {
                        Some(r) => success_result(op, Some(r.id.to_string()), Some(Utc::now().to_rfc3339())),
            ...
            _ => error_result(op, &format!("Unknown operation for question: {}", op.operation)),
        }
    }
}
