...
use super::result_helpers::{error_result, success_result, parse_uuid_field};
...
impl SubmissionPushDelegate {
    async fn handle_assessment_submission(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
    ...
    async fn handle_assignment_submission(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
        match op.operation.as_str() {
            "create" => {
            ...
            "grade" => {
                ...
                let score = match parse_uuid_field(&op.payload, "score") {
                    ...
                    Err(_) => {
                        ...
                        let score_val = op.payload.get("score").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
                        score_val
                    }
                };
    ...
    async fn handle_submission_file(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
        match op.operation.as_str() {
            ...
            _ => error_result(op, &format!("Unknown operation for submission_file: {}", op.operation)),
        }
    }
    ...
    async fn handle_material_file(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
        match op.operation.as_str() {
            ...
            _ => error_result(op, &format!("Unknown operation for material_file: {}", op.operation)),
        }
    }
}
