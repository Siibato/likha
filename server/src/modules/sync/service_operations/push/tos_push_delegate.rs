...
use crate::modules::tos::schema::{
    CreateCompetencyRequest, CreateTosRequest, UpdateCompetencyRequest, UpdateTosRequest,
};
...
impl TosPushDelegate {
    async fn handle_tos(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
        match op.operation.as_str() {
            "create" => {
                ...
                let classification_mode = match parse_str_field(&op.payload, "classification_mode")
                {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                let total_items = match parse_i32_field(&op.payload, "total_items") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                let time_unit = op
                ...
                let medium_percentage =
                    op.payload.get("medium_percentage").and_then(|v| v.as_f64());
            ...
            "update" => {
                let tos_id = match parse_uuid_field(&op.payload, "id") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                let request = UpdateTosRequest {
                    ...
                    remembering_percentage: op
                        .payload
                        .get("remembering_percentage")
                        .and_then(|v| v.as_f64()),
                    understanding_percentage: op
                        .payload
                        .get("understanding_percentage")
                        .and_then(|v| v.as_f64()),
                    applying_percentage: op
                        .payload
                        .get("applying_percentage")
                        .and_then(|v| v.as_f64()),
                    analyzing_percentage: op
                        .payload
                        .get("analyzing_percentage")
                        .and_then(|v| v.as_f64()),
                    evaluating_percentage: op
                        .payload
                        .get("evaluating_percentage")
                        .and_then(|v| v.as_f64()),
                    creating_percentage: op
                        .payload
                        .get("creating_percentage")
                        .and_then(|v| v.as_f64()),
                };
    ...
    async fn handle_tos_competency(&self, user_id: Uuid, op: &SyncQueueEntry) -> OperationResult {
        match op.operation.as_str() {
            "create" => {
                let client_id = parse_uuid_field(&op.payload, "id").ok();
                let tos_id = match parse_uuid_field(&op.payload, "tos_id") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                let competency_text = match parse_str_field(&op.payload, "competency_text") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                let time_units_taught = match parse_i32_field(&op.payload, "time_units_taught") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                ...
                let request = CreateCompetencyRequest {
                    ...
                    medium_count,
                    hard_count,
                    remembering_count: op
                        .payload
                        .get("remembering_count")
                        .and_then(|v| v.as_i64())
                        .map(|v| v as i32),
                    understanding_count: op
                        .payload
                        .get("understanding_count")
                        .and_then(|v| v.as_i64())
                        .map(|v| v as i32),
                    applying_count: op
                        .payload
                ...
                let result = if let Some(cid) = client_id {
                    ...
                    self.tos_service
                        .add_competency(tos_id, user_id, request)
                        .await
                };
            ...
            "update" => {
                let competency_id = match parse_uuid_field(&op.payload, "id") {
                    Ok(v) => v,
                    Err(e) => return error_result(op, &e),
                };
                ...
                let request = UpdateCompetencyRequest {
                    ...
                    medium_count,
                    hard_count,
                    remembering_count: op.payload.get("remembering_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
                    ...
                    understanding_count: op.payload.get("understanding_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
                    ...
                    applying_count: op.payload.get("applying_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
                    ...
                    analyzing_count: op.payload.get("analyzing_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
                    ...
                    evaluating_count: op.payload.get("evaluating_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
                    ...
                    creating_count: op.payload.get("creating_count").map(|v| {
                        if v.is_null() {
                            None
                        } else {
                            v.as_i64().map(|n| n as i32)
            ...
            "delete" => {
                ...
                match self
                    .tos_service
                    .delete_competency(competency_id, user_id)
                    .await
                {
                    Ok(_) => success_result(op, None, Some(Utc::now().to_rfc3339())),
                    Err(e) => error_result(op, &e.to_string()),
