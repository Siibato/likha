...
pub async fn get_learner_details(
    ...
    if !service.class_repo.is_teacher_of_class(auth_user.user_id, class_id).await.unwrap_or(false) {
        return (
            StatusCode::FORBIDDEN,
            Json(MessageResponse { message: "Access denied".to_string() }),
        ).into_response();
    }
    match service.repo.get_learner_details(student_id).await {
        Ok(Some(model)) => success_response(LearnerDetailsResponse::from(model), StatusCode::OK).into_response(),
        Ok(None) => (
            StatusCode::NOT_FOUND,
            Json(MessageResponse { message: "Learner details not found".to_string() }),
        ).into_response(),
...
pub async fn upsert_learner_details(
    ...
    match service.repo.upsert_learner_details(
        student_id,
        req.lrn,
        req.age,
        req.sex,
...
pub async fn get_attendance(
    ...
    match service.repo.get_attendance(student_id, q_class_id, query.school_year.as_deref()).await {
        Ok(records) => success_response(
            records.into_iter().map(AttendanceResponse::from).collect::<Vec<_>>(),
            StatusCode::OK,
        ).into_response(),
        Err(e) => e.into_response(),
    }
...
pub async fn upsert_attendance(
    ...
    let req_class_id = match Uuid::parse_str(&req.class_id) {
        ...
        Err(_) => return (
            ...
            Json(MessageResponse { message: "Invalid class_id".to_string() }),
        ).into_response(),
    };
    match service.repo.upsert_attendance(
        student_id,
        req_class_id,
        req.school_year,
        req.month,
...
pub async fn get_core_values(
    ...
    let q_class_id = query.class_id.and_then(|s| Uuid::parse_str(&s).ok());
    match service.repo.get_core_values(student_id, q_class_id, query.school_year.as_deref()).await {
        Ok(records) => success_response(
            records.into_iter().map(CoreValuesResponse::from).collect::<Vec<_>>(),
            StatusCode::OK,
        ).into_response(),
        Err(e) => e.into_response(),
    }
...
pub async fn upsert_core_values(
    ...
    match service.repo.upsert_core_values(
        student_id,
        req_class_id,
        req.school_year,
...
pub async fn get_school_history(
    ...
    match service.repo.get_school_history(student_id).await {
        Ok(records) => success_response(
            records.into_iter().map(SchoolHistoryResponse::from).collect::<Vec<_>>(),
            StatusCode::OK,
        ).into_response(),
        Err(e) => e.into_response(),
    }
...
pub async fn create_school_history(
    ...
    match service.repo.create_school_history(
        student_id,
        req.school_name,
        req.school_id,
        req.grade_level,
        ...
        Ok(model) => success_response(SchoolHistoryResponse::from(model), StatusCode::CREATED).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn update_school_history(
    ...
    match service.repo.update_school_history(
        history_id,
        req.school_name,
        req.school_id,
        req.grade_level,
        ...
        Ok(model) => success_response(SchoolHistoryResponse::from(model), StatusCode::OK).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn delete_school_history(
    ...
    match service.repo.delete_school_history(history_id).await {
        Ok(()) => success_response(MessageResponse { message: "Deleted".to_string() }, StatusCode::OK).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn upsert_previous_subject(
    ...
    let req_history_id = match Uuid::parse_str(&req.school_history_id) {
        ...
        Err(_) => return (
            StatusCode::BAD_REQUEST,
            Json(MessageResponse { message: "Invalid school_history_id".to_string() }),
        ).into_response(),
    };
    match service.repo.upsert_previous_subject(
        student_id,
        req_history_id,
        req.subject_name,
        req.subject_group,
...
pub async fn get_previous_attendance(
    ...
    let q_history_id = query.class_id.and_then(|s| Uuid::parse_str(&s).ok());
    match service.repo.get_previous_attendance(student_id, q_history_id).await {
        Ok(records) => success_response(
            records.into_iter().map(PreviousAttendanceResponse::from).collect::<Vec<_>>(),
            StatusCode::OK,
        ).into_response(),
        Err(e) => e.into_response(),
    }
...
pub async fn upsert_previous_attendance(
    ...
    match service.repo.upsert_previous_attendance(
        student_id,
        req_history_id,
        req.school_year,
        req.month,
...
pub async fn get_sf10(
    ...
    match service.get_sf10(class_id, student_id, auth_user.user_id).await {
