...
impl crate::modules::assessment::service::AssessmentService {
    pub async fn grade_essay_answer(
        ...
        let submission = self
            .assessment_repo
            .find_submission_by_id(answer.submission_id)
            .await?
        ...
        if question.question_type != "essay" {
            return Err(AppError::BadRequest(
                "This endpoint is only for essay questions".to_string(),
            ));
        }
        ...
        if let Some(ref inv) = self.invalidator {
            inv.invalidate_assessment_submissions(submission.assessment_id)
                .await;
            inv.invalidate_assessment_submission_detail(submission.id)
                .await;
