...
impl crate::modules::assessment::service::AssessmentService {
    pub async fn update_question(
        ...
        let assessment = self
            .assessment_repo
            .find_by_id(question.assessment_id)
            .await?
        ...
        if assessment.is_published {
            return Err(AppError::BadRequest(
                "Cannot edit questions in a published assessment".to_string(),
            ));
        }
        ...
        if let Some(choices) = request.choices {
            self.assessment_repo
                .delete_choices_by_question_id(question_id)
                .await?;
        ...
        if let Some(answers) = request.correct_answers {
            self.assessment_repo
                .delete_correct_answers_by_question_id(question_id)
                .await?;
            for answer in answers {
                self.assessment_repo
                    .add_correct_answer(question_id, answer)
                    .await?;
            }
        }
        ...
        if let Some(items) = request.enumeration_items {
            self.assessment_repo
                .delete_all_answer_keys_for_question(question_id)
                .await?;
            for item in items {
                self.assessment_repo
        ...
        self.assessment_repo
            .update_total_points(question.assessment_id)
            .await?;

        if let Some(ref inv) = self.invalidator {
            inv.invalidate_assessment_detail(question.assessment_id)
                .await;
        }
