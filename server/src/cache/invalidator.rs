...
impl CacheInvalidator {
    ...
    pub async fn invalidate_student_classes(&self, student_id: Uuid) {
        self.cache
            .del(&CacheKey::ClassListStudent(student_id).as_str())
            .await;
        self.cache
            .del(&CacheKey::ClassMetadata(student_id, "student".to_string()).as_str())
            .await;
    }

    pub async fn invalidate_teacher_classes(&self, teacher_id: Uuid) {
        self.cache
            .del(&CacheKey::ClassListTeacher(teacher_id).as_str())
            .await;
        self.cache
            .del(&CacheKey::ClassMetadata(teacher_id, "teacher".to_string()).as_str())
            .await;
    }

    pub async fn invalidate_student_assignments(&self, student_id: Uuid) {
        self.cache
            .del(&CacheKey::AssignmentListStudent(student_id).as_str())
            .await;
    }

    pub async fn invalidate_teacher_assignments(&self, teacher_id: Uuid) {
        self.cache
            .del(&CacheKey::AssignmentListTeacher(teacher_id).as_str())
            .await;
    }

    pub async fn invalidate_assessments(&self, user_id: Uuid, class_id: Uuid) {
        self.cache
            .del(&CacheKey::AssessmentList(user_id, class_id).as_str())
            .await;
    }
    ...
    pub async fn invalidate_class_participants(&self, class_id: Uuid) {
        self.cache
            .del(&CacheKey::ClassParticipants(class_id).as_str())
            .await;
    }
    ...
    pub async fn invalidate_all_class_grades(&self, class_id: Uuid) {
        let keys = vec![CacheKey::GeneralAverages(class_id).as_str()];
        self.cache.del_keys(keys).await;
    }

    pub async fn invalidate_item_scores(&self, item_id: Uuid) {
        self.cache
            .del(&CacheKey::ItemScores(item_id).as_str())
            .await;
    }
    ...
    pub async fn invalidate_assessment_submissions(&self, assessment_id: Uuid) {
        self.cache
            .del(&CacheKey::AssessmentSubmissions(assessment_id).as_str())
            .await;
    }

    pub async fn invalidate_assessment_submission_detail(&self, submission_id: Uuid) {
        self.cache
            .del(&CacheKey::AssessmentSubmissionDetail(submission_id).as_str())
            .await;
    }

    pub async fn invalidate_student_results(&self, submission_id: Uuid) {
        self.cache
            .del(&CacheKey::StudentResults(submission_id).as_str())
            .await;
    }

    pub async fn invalidate_assessment_student_submission(
        &self,
        assessment_id: Uuid,
        student_id: Uuid,
    ) {
        self.cache
            .del(&CacheKey::AssessmentStudentSubmission(assessment_id, student_id).as_str())
            .await;
    }

    pub async fn invalidate_student_assessment_submissions(
        &self,
        class_id: Uuid,
        student_id: Uuid,
    ) {
        self.cache
            .del(&CacheKey::StudentAssessmentSubmissions(class_id, student_id).as_str())
            .await;
    }
    ...
    pub async fn invalidate_user_profile(&self, user_id: Uuid) {
        self.cache
            .del(&CacheKey::UserProfile(user_id).as_str())
            .await;
    }
    ...
    pub async fn invalidate_material_list(&self, class_id: Uuid) {
        self.cache
            .del(&CacheKey::MaterialList(class_id).as_str())
            .await;
    }

    pub async fn invalidate_material_detail(&self, material_id: Uuid) {
        self.cache
            .del(&CacheKey::MaterialDetail(material_id).as_str())
            .await;
    }
