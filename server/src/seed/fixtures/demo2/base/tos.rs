//! Demo-2 TOS: 3 terms for 7 classes (1 Advisory + 6 subjects).

use super::super::{cid, tid};
use crate::seed::specs::TosSpec;

pub fn demo2_tos() -> Vec<TosSpec> {
    let mut tos_list = Vec::with_capacity(21);
    ...
    // English 10 - T1–T3 (bloom template, WW 50%, PT 30%, QA 20%)
    for term in 1..=3 {
        tos_list.push(TosSpec {
    ...
    // AP 10 - T1–T3 (bloom template, WW 50%, PT 30%, QA 20%)
    for term in 1..=3 {
        tos_list.push(TosSpec {
    ...
    // Filipino 10 - T1–T3 (bloom template, WW 50%, PT 30%, QA 20%)
    for term in 1..=3 {
        tos_list.push(TosSpec {
    ...
    // TLE 10 - T1–T3 (bloom template, WW 50%, PT 30%, QA 20%)
    for term in 1..=3 {
        tos_list.push(TosSpec {
