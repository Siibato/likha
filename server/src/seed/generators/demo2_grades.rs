//! Demo-2 grade generators that derive scores from actual submissions.
...
/// Generate grade records for each class × each term (3 terms for demo-2).
pub fn generate_demo2_grade_records(classes: &[ClassSpec]) -> Vec<GradeRecordSpec> {
    let mut records = Vec::new();
    for class in classes {
        if class.deleted_at.is_some() {
            continue;
        }
        // Demo-2 uses standard DepEd weights: 40% WW, 40% PT, 20% QA for all subjects
        let (ww, pt, qa) = (40.0, 40.0, 20.0);
        for term in 1..=3 {
...
pub fn generate_demo2_grade_items(
    ...
    for a in assessments {
        ...
        let id = crate::seed::tools::seed_id(
            "grade_items",
            &format!("assess_{}_{}", a.id, a.term_number),
        );
    ...
    for a in assignments {
        ...
        let id = crate::seed::tools::seed_id(
            "grade_items",
            &format!("assign_{}_{}", a.id, a.term_number),
        );
        items.push(GradeItemSpec {
            id,
            class_id: a.class_id,
...
pub fn generate_demo2_grade_scores(
    ...
    for enrollment in enrollments {
        if students
            .iter()
            .any(|s| s.id == enrollment.user_id && s.role == "student")
        {
            class_students
...
pub fn generate_demo2_term_grades(
    ...
    for record in grade_records {
        ...
        for (_student_idx, student_id) in enrolled.iter().enumerate() {
            let student_scores: Vec<&GradeScoreSpec> = grade_scores
                .iter()
