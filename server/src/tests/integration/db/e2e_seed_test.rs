...
async fn test_e2e_seed_produces_expected_row_counts() {
    ...
    let assessments = ::entity::assessments::Entity::find()
        .all(&db)
        .await
        .expect("find assessments");
    assert_eq!(
        assessments.len(),
        5,
        "expected 5 assessments (incl. deleted)"
    );
    ...
    assert_eq!(
        questions.len(),
        25,
        "expected 25 assessment questions (5 per assessment)"
    );
    ...
    let assignments = ::entity::assignments::Entity::find()
        .all(&db)
        .await
        .expect("find assignments");
    assert_eq!(
        assignments.len(),
        4,
        "expected 4 assignments (incl. deleted)"
    );
    ...
    let grade_records = ::entity::grade_record::Entity::find()
        .all(&db)
        .await
        .expect("find grade records");
    // setup_defaults creates 4 terms × 2 classes = 8 records
    ...
    let t1 = ::entity::users::Entity::find_by_id(TEACHER_01_ID)
        .one(&db)
        .await
        .expect("find teacher_01")
        .expect("teacher_01 must exist");
    ...
    let math_class = ::entity::classes::Entity::find_by_id(CLASS_MATH_8A_ID)
        .one(&db)
        .await
        .expect("find math class")
        .expect("math class must exist");
    ...
    let assess = ::entity::assessments::Entity::find_by_id(ASSESS_MATH_QUIZ1_ID)
        ...
        .await
        .expect("find quiz1")
        .expect("quiz1 must exist");
    assert!(assess.is_published, "Math Quiz 1 must be published");
    assert!(
        assess.results_released,
        "Math Quiz 1 must have results released"
    );
    ...
    let quiz2 = ::entity::assessments::Entity::find_by_id(ASSESS_MATH_QUIZ2_ID)
        ...
        .await
        .expect("find quiz2")
        .expect("quiz2 must exist");
    assert!(quiz2.is_published, "Math Quiz 2 must be published");
    assert!(
        !quiz2.results_released,
        "Math Quiz 2 results must NOT be released"
    );

    let deleted_student = ::entity::users::Entity::find_by_id(STUDENT_DELETED_ID)
        .one(&db)
        .await
        .expect("find deleted student")
        .expect("deleted student must exist");
    assert!(
        deleted_student.deleted_at.is_some(),
        "student_deleted_99 must have deleted_at set"
    );
}
...
async fn test_e2e_seed_soft_deleted_entities_have_deleted_at() {
    ...
    let deleted_class = ::entity::classes::Entity::find_by_id(CLASS_DELETED_8_ID)
        .one(&db)
        .await
        .expect("find deleted class")
        .expect("deleted class must exist");
    assert!(
        deleted_class.deleted_at.is_some(),
        "deleted class must have deleted_at"
    );

    let deleted_assess = ::entity::assessments::Entity::find_by_id(ASSESS_DELETED_MATH_ID)
        .one(&db)
        .await
        .expect("find deleted assess")
        .expect("deleted assess must exist");
    assert!(
        deleted_assess.deleted_at.is_some(),
        "deleted assessment must have deleted_at"
    );

    let deleted_assign = ::entity::assignments::Entity::find_by_id(ASSIGN_DELETED_ID)
        .one(&db)
        .await
        .expect("find deleted assign")
        .expect("deleted assign must exist");
    assert!(
        deleted_assign.deleted_at.is_some(),
        "deleted assignment must have deleted_at"
    );
}
