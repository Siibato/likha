...
ResultFuture<List<Assessment>> getAssessments(
  ...
  if (forceRemote && base.serverReachabilityService.isServerReachable) {
    try {
      RepoLogger.instance.log('getAssessments() - forceRemote=true, fetching from remote for classId: $classId');
      final fresh = await base.remoteDataSource.getAssessments(classId: classId);
      await base.localDataSource.cacheAssessments(fresh);
  ...
  try {
    try {
      ...
      final cachedAssessments = await base.localDataSource.getCachedAssessments(classId, publishedOnly: publishedOnly);
      RepoLogger.instance.log('getAssessments() - loaded ${cachedAssessments.length} cached assessments');

      String? currentStudentId;
      try {
        currentStudentId = await base.storageService.getUserId();
      ...
      for (final assessment in cachedAssessments) {
        try {
          final actualSubmissionCount = await base.localDataSource.getCachedSubmissionCount(assessment.id);
          bool? isSubmitted;

          if (currentStudentId != null) {
            try {
              isSubmitted = await base.localDataSource.hasStudentSubmittedAssessment(
                assessment.id,
                currentStudentId,
              );
            } catch (e) {
      ...
      if (!skipBackgroundRefresh) {
        _backgroundFetchAssessments(base, classId, publishedOnly: publishedOnly);
      }
...
void _backgroundFetchAssessments(AssessmentRepositoryBase base, String classId, {bool publishedOnly = false}) {
  Future.microtask(() async {
    try {
      RepoLogger.instance.log('_backgroundFetchAssessments() - fetching fresh assessments for classId: $classId');
      final fresh =
          await base.remoteDataSource.getAssessments(classId: classId);
      RepoLogger.instance.log('_backgroundFetchAssessments() - received ${fresh.length} fresh assessments');

      final List<Assessment> cached;
      try {
        cached = await base.localDataSource.getCachedAssessments(classId, publishedOnly: publishedOnly);
        ...
        await base.localDataSource.cacheAssessments(fresh);
        base.dataEventBus.notifyAssessmentsChanged(classId);
        return;
      }
