...
class HistoryImportDialog extends ConsumerStatefulWidget {
  final String type;
  final String title;
  final VoidCallback? onSuccess;

  const HistoryImportDialog({
    super.key,
    required this.type,
    required this.title,
    this.onSuccess,
  ...
  @override
  ConsumerState<HistoryImportDialog> createState() => _HistoryImportDialogState();
}

class _HistoryImportDialogState extends ConsumerState<HistoryImportDialog> {
  ...
  static const _instructions = {
    'school_history': 'Upload school history records for existing students. '
        'Required: username, school_name, grade_level, school_year. '
        'The student must already have an account.',
    'subjects': 'Upload previous subject grades. Requires matching school history '
        '(username + school_name + school_year must already exist).',
    'attendance': 'Upload monthly attendance records. Requires matching school history '
        '(username + school_name + school_year must already exist).',
  };
  ...
  Future<void> _preview() async {
    ...
    await ref.read(importProvider.notifier).previewHistoryImport(
  ...
  Future<void> _import() async {
    await ref.read(importProvider.notifier).importHistory(widget.type);
  ...
  Widget build(BuildContext context) {
    ...
    final columns = _columnMap[widget.type]!;
  ...
  Widget _buildContent(ImportState state, List<String> columns) {
    switch (_step) {
      case _ImportStep.upload:
        return _buildUploadStep(state);
      case _ImportStep.preview:
        return _buildPreviewStep(state, columns);
      ...
      children: [
        Container(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Icon(Icons.info_outline_rounded, size: 18, color: AppColors.foregroundSecondary),
  ...
  Widget _buildPreviewStep(ImportState state, List<String> columns) {
