...
class GradeDataRow extends StatelessWidget {
  ...
  // Inline score editing state (owned by GradeSpreadsheet)
  ...
  Widget build(BuildContext context) {
    ...
    final available = [wwStats.ws, ptStats.ws, qaStats.ws].whereType<double>().toList();
      ...
      child: Row(
        children: [
          ...
          GradeComputedCell(
            ...
            bold: true,
            color: initialGrade != null ? null : null,
          ),
          if (isEditingQg)
            GradeInlineEditCell(
              ctrl: qgCtrl,
          ...
          GradeRemarksCell(remarks: remarks, bgColor: bgColor),
        ],
      ),
    );
  }
  ...
  GradeScoreStats _computeStats(String sid, List<GradeItem> items, double weight) {
    final studentScores = scoreLookup[sid] ?? {};
    double total = 0;
    double hs = 0;
    bool hasScore = false;
    ...
    return GradeScoreStats(total: total, hs: hs, pct: pct, ws: pct * weight / 100);
  }
